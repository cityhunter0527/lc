...
class RBTree {
    class TreeNode {
        public:
            ...
            TreeNode(T x, void* blob):val(x), color(RED), data(blob), left(nullptr), right(nullptr), parent(nullptr) {}
    };
...
protected:
    ...
    TreeNode* BSTLookup(TreeNode*, const T key); 
...
public:
    RBTree():root(nullptr) {};
    void insert(const T key, void* data); // << geeksforgeeks
    void remove(const T key); // << <<Introduction to Algorithms>>
    vector<T> inOrder();
    vector<T> levelOrder();
    vector<vector<string>> printTree();
    TreeNode* getRoot() {return root;}
    void* lookup(const T key);
...
//
template <typename T>
typename RBTree<T>::TreeNode* RBTree<T>::BSTInsert(TreeNode* n, TreeNode* p) {
...
//
template <typename T>
void RBTree<T>::fixViolation(TreeNode* x) {
...
//T1  T2                       T2  T3
template <typename T>
void RBTree<T>::rightRotate(TreeNode* p) {
...
//    T2  T3             T1  T2
template <typename T>
void RBTree<T>::leftRotate(TreeNode* p) {
    TreeNode* parent = p->parent;
    TreeNode* x = p->right;
...
template <typename T>
void RBTree<T>::transplant(TreeNode* u, TreeNode* v) {
    assert (u != nullptr);
    if (u->parent == nullptr)
        root = v;
...
template <typename T>
typename RBTree<T>::TreeNode* RBTree<T>::BSTLookup(TreeNode* r, const T key) {
...
template <typename T>
void RBTree<T>::removeFixup(TreeNode* x, TreeNode* px) {
    TreeNode* w = nullptr; 
...
template <typename T>
typename RBTree<T>::TreeNode* RBTree<T>::minimum(TreeNode* u) {
...
template <typename T>
void RBTree<T>::insert(const T key, void* data) {
...
template <typename T> 
void* RBTree<T>::search(const T key) {
...
template <typename T>
void* RBTree<T>::lookup(const T key) {
...
template <typename T>
vector<vector<string>> RBTree<T>::printTree() {
